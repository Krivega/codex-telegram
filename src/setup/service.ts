...
export function servicePlist(directory: string): string {
...
<key>EnvironmentVariables</key><dict><key>PATH</key><string>${xml([dirname(process.execPath), '/usr/local/bin', '/usr/bin', '/bin'].join(':'))}</string></dict>
...
export async function installService(directory: string): Promise<void> {
  ...
  await writeFile(path, servicePlist(directory), { mode: 0o600 });
