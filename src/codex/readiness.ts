...
export async function checkCodexReadiness(codex: CodexPort, threadIds: readonly string[], options: { allowEmpty?: boolean } = {}): Promise<number> {
  const threads = await codex.listThreads();
  const selected = threadIds.length ? [...new Set(threadIds)].filter((id) => codex.permitsThread?.(id) ?? true).map((id) => {
  ...
  if (!selected.length && !options.allowEmpty) throw new Error('Нет открытых задач, принимающих сообщения. Откройте задачу в исходном клиенте и проверьте подключение к его серверу.');
...
export async function ensureCodexStartup(directory: string, config: Config | AgentConfig, codex: CodexPort): Promise<void> {
  ...
  await checkCodexReadiness(codex, config.codex.threadIds, { allowEmpty: config.codex.transport === 'desktop' && !config.codex.threadIds.length });
