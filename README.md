...
Создайте бота через `@BotFather` в Telegram и сохраните выданный токен для ввода в мастер. Мастер сам ищет Codex среди команд терминала, в `~/.local/bin`, а на macOS — также внутри приложений ChatGPT и Codex в `/Applications` и `~/Applications`.
...
3. Путь к программе Codex. Если мастер нашёл программу, нажмите Enter, чтобы сохранить предложенный абсолютный путь. При необходимости можно указать другой путь вручную.
...
**Если `command -v codex` ничего не выводит.** Терминал не нашёл команду в своём списке каталогов `PATH`. При этом CLI может уже находиться внутри установленного приложения. На macOS мастер проверяет, в частности, `/Applications/ChatGPT.app/Contents/Resources/codex` и `/Applications/Codex.app/Contents/Resources/codex`. Редактировать `PATH` для нашей программы не требуется: найденный полный путь сохраняется в настройках. Если ни одна программа не найдена, установите CLI по [официальной инструкции OpenAI](https://learn.chatgpt.com/docs/codex/cli) и повторите настройку. Наличие исполняемого файла ещё не подтверждает доступ к исходным задачам — это проверяется следующим шагом.
...
Если проверка сообщает, что сокет не найден или этот сервер не обслуживает открытую задачу приложения, связь с Codex ещё не готова и службу запускать нельзя. Нужен поддерживаемый приложением доступ к серверу исходной задачи по разделу [«Подключение к настольному Codex»](#подключение-к-настольному-codex). Не задавайте внутренние переменные настольного клиента и не запускайте отдельный демон как обход: это локальная проблема совместимости, которую адрес в интернете и туннель не устраняют. Проверенный сбой, восстановление и условия безопасной повторной проверки записаны в [разборе совместимости](docs/codex-desktop-compatibility.md).
...
На компьютерах с Codex дополнительно нужен поддерживаемый приложением App Server с Unix-сокетом, который обслуживает нужные исходные задачи. Наличие CLI или отдельно запущенного сервера само по себе не подтверждает связь с окном приложения.
...
Служба не меняет параметры запуска настольного Codex. Внутренние переменные приложения, включая принудительный адрес WebSocket, не являются поддерживаемым способом настройки этого проекта: ошибка такого адреса может заблокировать запуск самого приложения. Если текущая версия Codex не предоставляет общий сервер штатно, интеграция остаётся недоступной до появления совместимого способа подключения. На проверенной версии именно это ограничение сейчас блокирует запуск; подробности, аварийный откат и критерии повторной проверки — в [docs/codex-desktop-compatibility.md](docs/codex-desktop-compatibility.md).
...
Команды выполняются на соответствующих машинах; при двух ролях на одной машине устанавливаются две службы `launchd`. Для компьютера сохраните абсолютный `codex.executable`, найденный мастером настройки, или укажите путь вручную. Удаление: `service uninstall` с тем же `--home`. После перемещения проекта или смены Node.js переустановите службу. Узел должен оставаться включённым: его сон приостанавливает связь сразу со всеми компьютерами. Установка и запуск служб пока требуют отдельной проверки.
