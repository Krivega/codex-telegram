...
**Версия 0.3.2 (экспериментальный Desktop).** Сценарий двух компьютеров проверен автоматически через настоящий локальный HTTP-сервер и независимые базы. Codex и Telegram в этих проверках заменены имитаторами. Новый режим `desktop` проверен с двумя исходными задачами приложения и настоящим PDF; Telegram в этом опыте заменён проверочным получателем. Установка плагина, расписание, настоящий бот и два физических компьютера ещё требуют [сквозной проверки](docs/acceptance.md).

Новые основные задачи Desktop подхватываются без перезапуска. Диспетчер регистрируется плагином и исключается из уведомлений; старые итоги до первого включения не рассылаются.

**Для текущего настольного Codex начните с [инструкции Desktop](docs/desktop-mode.md)**: сокет Codex не нужен, поручения передаёт плагин через встроенные инструменты приложения.
...
Понадобятся Node.js 24 или новее и отдельный Telegram-бот. Выберите `desktop` для связи через плагин либо `unix` при наличии общего сервера исходных задач. Отдельная установка Codex CLI для нашей службы не требуется. Границы проведённого опыта описаны в инструкции Desktop.
...
Создайте бота через `@BotFather` в Telegram и сохраните выданный токен для ввода в мастер. Для приложения выберите Desktop; для режима unix нужен адрес уже работающего общего сервера.
...
3. Способ подключения: `desktop` (по умолчанию) или `unix`. Для Desktop — каталог журналов; для unix — путь общего сокета.
4. Задачи: `all` по умолчанию — автоматическое обнаружение существующих и новых задач. Идентификаторы через запятую нужны только для ограничения набора.
5. Интервал проверки. По умолчанию — пять секунд.
...
**Если `command -v codex` ничего не выводит.** Для Codex Telegram это не препятствие: служба использует плагин Desktop либо обращается к сокету напрямую; CLI не запускается. Устанавливать CLI или менять `PATH` ради нашей службы не нужно. Старое поле `codex.executable` принимается для совместимости, но не используется; его можно удалить. Доступ к исходным задачам всё равно нужно проверить следующим шагом.

### 3. Проверить подключение и запустить

Для `desktop` сначала подготовьте плагин и диспетчер по [инструкции](docs/desktop-mode.md). Следующие пояснения об общем сокете относятся только к `unix`.
...
Если проверка сообщает, что сокет не найден или выбранные задачи не открыты и не принимают сообщения, первоначальное подключение ещё не готово. `start` повторяет проверку перед первым запуском и при отказе не создаёт базу и не начинает получать сообщения Telegram. Нужен поддерживаемый приложением доступ к серверу исходной задачи по разделу [«Подключение к настольному Codex»](#подключение-к-настольному-codex). Не задавайте внутренние переменные настольного клиента и не запускайте отдельный демон как обход. Наблюдения, восстановление и условия повторной проверки записаны в [разборе совместимости](docs/codex-desktop-compatibility.md).

Успешная первоначальная проверка сохраняется в личном каталоге в `codex-readiness.json`. При тех же настройках программа может перезапускаться во время временного отключения Codex и сохранять поручения до восстановления связи. Смена сокета, выбранных задач или привязки компьютера требует новой проверки. `doctor` всегда проверяет текущее соединение заново. Результат проверки протокола не подтверждает, что сообщение появится в нужном окне: перед постоянной работой выполните ручную проверку ниже.
...
На компьютерах с Codex выберите Desktop с локальным плагином либо поддерживаемый приложением App Server с Unix-сокетом исходных задач. Наличие CLI или отдельно запущенного сервера само по себе не подтверждает связь с окном приложения.
...
Мастер спросит адрес узла, код подключения, способ подключения Codex и выбранные задачи. При Desktop каждому компьютеру нужны свой плагин и диспетчер. Токен Telegram на компьютер не передаётся. Собственный случайный ключ сохраняется до запроса привязки: повтор после потерянного ответа использует ту же регистрацию.
...
Новый вариант `desktop` описан в [отдельной инструкции](docs/desktop-mode.md). Ниже — прежний транспорт `unix` и его ограничения.

Служба подключается напрямую к Unix-сокету App Server: выполняет стандартный HTTP Upgrade и передаёт по одному сообщению JSON-RPC в каждом текстовом WebSocket-кадре. Байтовый `app-server proxy` для этого не используется. Служба не запускает отдельный сервер, не копирует историю и не создаёт задачу вместо исходной. Стандартный сокет: `$CODEX_HOME/app-server-control/app-server-control.sock`, обычно внутри `~/.codex`.

`doctor` проверяет историю и очередь каждой явно выбранной задачи; отсутствующая или закрытая задача вызывает отказ. При `all` проверяются все открытые задачи, принимающие сообщения; нужна хотя бы одна такая задача. Для отправки нужна задача, уже загруженная тем же сервером и сообщающая `canAcceptDirectInput: true`. Сохранённую, но не открытую задачу нужно открыть в её исходном клиенте. Если сокет отсутствует, требуется выяснить доступный способ подключения именно вашей версии приложения. Запуск второго независимого сервера не доказывает связь с открытым окном.

Служба не меняет параметры запуска настольного Codex. Внутренние переменные приложения, включая принудительный адрес WebSocket, не являются поддерживаемым способом настройки этого проекта: ошибка такого адреса может заблокировать запуск самого приложения. Если текущая версия Codex не предоставляет общий сервер штатно, транспорт `unix` для исходных задач остаётся недоступным. Для этой конфигурации добавлен экспериментальный Desktop; подробности, аварийный откат и критерии повторной проверки — в [docs/codex-desktop-compatibility.md](docs/codex-desktop-compatibility.md).
...
Команды выполняются на соответствующих машинах; при двух ролях на одной машине устанавливаются две службы `launchd`. Установка использует текущий Node.js и не требует пути к Codex CLI. Удаление: `service uninstall` с тем же `--home`. После перемещения проекта или смены Node.js переустановите службу. Узел должен оставаться включённым: его сон приостанавливает связь сразу со всеми компьютерами. Установка и запуск служб пока требуют отдельной проверки.
...
WebSocket использует библиотеку `ws`; HTTP, TLS, криптография и SQLite предоставлены Node.js. Зависимости, TypeScript и типы закреплены в `package-lock.json`; SQLite в проверенной версии Node.js сообщает об экспериментальном статусе. Linux и Windows пока не проверены.
